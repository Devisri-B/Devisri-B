...
def add_scaled_labels(ax, rects, labels, sizes):
    ...
    for rect, label, norm_size in zip(rects, labels, norm_sizes):
        ...
        w = rect['dx']
        h = rect['dy']
        # Skip boxes that are too tiny to label
        if w < 5 or h < 5:
                        continue
                    # Scale font size based on box area (norm_size), clamped between 7 and 20
                    font_size = max(7, min(20, 20 * (norm_size ** 0.35)))
        cx = x + w / 2
        cy = y + h / 2
        # Split label into name and count lines
        parts = label.split('\n')
        name = parts[0]
        count_str = parts[1] if len(parts) > 1 else ''
        # Create a temporary text to check fit, then adjust
        txt = ax.text(
                        cx, cy, name,
                        ha='center', va='center',
        ...
        # Check if text fits; if not, reduce font size until it fits or hits minimum
        renderer = ax.get_figure().canvas.get_renderer()
        bbox = txt.get_window_extent(renderer=renderer)
        fig_bbox = ax.get_figure().get_window_extent(renderer=renderer)
        ax_bbox = ax.get_window_extent(renderer=renderer)
        # Convert box width/height from data to display coords
        ...
        # Shrink font until text fits or reaches min size
        while font_size > 7:
                        bbox = txt.get_window_extent(renderer=renderer)
        ...
        # Add count below name if there's enough height
...
def generate_chart():
    ...
    # Compute squarify rectangles manually so we can label them ourselves
    ...
    # Draw labels with dynamic font sizing
